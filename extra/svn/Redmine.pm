...
sub RedmineDSN { 
  ...
  $self->{RedmineDSN} = $arg;
  my $query = "SELECT 
                 hashed_password, auth_source_id, permissions, cas_id
...
sub is_member {
  ...
  my $ret;
  while (my ($hashed_password, $auth_source_id, $permissions, $cas_id) = $sth->fetchrow_array) {

      unless ($auth_source_id or $cas_id) {
	  my $method = $r->method;
      ...
      } elsif ($CanUseLDAPAuth and $cas_id) {
          my $sthldap = $dbh->prepare(
              "SELECT ldap,port,dn,attr_login from cas WHERE id = ?;"
          );
          $sthldap->execute($cas_id);
